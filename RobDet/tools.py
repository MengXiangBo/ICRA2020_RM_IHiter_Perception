...
CLASS_COLOR = [(np.random.randint(255),np.random.randint(255),np.random.randint(255)) for _ in range(len(RM_CLASSES))]
...
class BCELoss(nn.Module):
    ...
    def forward(self, inputs, targets, mask):
        ...
        pos_loss = -pos_id * (targets * torch.log(inputs + 1e-14))
...
def get_total_anchor_size(multi_level=False):
    if multi_level:
...
def gt_creator(input_size, stride, label_lists):
    ...
    all_anchor_size = get_total_anchor_size()
...
def multi_gt_creator(input_size, strides, label_lists=[]):
    ...
    # generate gt datas
    all_anchor_size = get_total_anchor_size(multi_level=True)
...
if __name__ == "__main__":
    # import os
    # i = 0
    # for _ in os.listdir('/home/k545/object-detection/myYOLO/data/coco/images/trainval35k/'):
    #     i += 1
    # print(i)
    dataset_root = './data/robots/'

    dataset = RMDetection(root=dataset_root,
                            transform=BaseTransform([448, 448],
                                                        MEANS))
    batch_iter = iter(dataset)
    for i in range(128):
        img = dataset.pull_image(i)
        # img, target = dataset[i]
        # img = img.permute(1,2,0).numpy().astype(np.uint8)[:,:,::-1]
        # cv2.imshow('img', img)
        # cv2.waitKey(0)
        ...
        for anno in target:
            xmin, ymin, xmax, ymax, label_ind = anno
            xmin = xmin / width * 448
            ymin = ymin / height * 448
            xmax = xmax / width * 448
            ymax = ymax / height * 448
            label_ind = int(label_ind)
            print((int(xmin), int(ymin)), (int(xmax), int(ymax)))

            cv2.rectangle(img, (int(xmin), int(ymin)), (int(xmax), int(ymax)),(0,255,0), 2)
            mess = RM_CLASSES[label_ind]
