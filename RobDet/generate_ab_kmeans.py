...
def anchor_box_kmeans(total_gt_boxes, n_anchors, loss_convergence, iters, plus=True):
    ...
    for centroid in centroids:
        print('w: %.2f, h: %.2f, area: %.2f' % (centroid.w, centroid.h, centroid.w*centroid.h))
...
if __name__ == "__main__":
    ...
    stride = 1
    dataset = RMDetection(root=RM_ROOT,
    ...
    for i in range(len(dataset)):
        ...
        for box_and_label in annotation:
            ...
            # For YOLOv2 and SlimYOLOv2, we divide each box by stride
            ...
            # For TinyYOLOv3, we don't do that. It's up to you.
            # bx = (xmin + xmax) / 2 / w * input_size[1]
            # by = (ymin + ymax) / 2 / h * input_size[0]
            # bw = (xmax - xmin) / w * input_size[1]
            # bh = (ymax - ymin) / h * input_size[0]
